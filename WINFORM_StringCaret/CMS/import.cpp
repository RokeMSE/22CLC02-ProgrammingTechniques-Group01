...
bool importStudents() {
    std::ifstream ifs("CSV/Student.csv");
    ...
    while (!ifs.eof()) {
        ...
        System::String^ tmpStr = marshal_as<System::String^>(str);
        tmp->user->username = tmpStr;

        std::getline(ifs, str, ','); // get password
        tmpStr = marshal_as<System::String^>(str);
        tmp->user->password = System::String(str.c_str());
        ...
        std::getline(ifs, str, ',');
        tmpStr = marshal_as<System::String^>(str);
        tmp->studentID = System::String(str.c_str());

        tmpStr = marshal_as<System::String^>(str);
        std::getline(ifs, str, ',');
        tmp->firstname = System::String(str.c_str());

        std::getline(ifs, str, ',');
        tmpStr = marshal_as<System::String^>(str);
        tmp->lastname = System::String(str.c_str());
        ...
        std::getline(ifs, str, ',');
        tmpStr = marshal_as<System::String^>(str);
        tmp->DoB = new GROUP1::DATE;
        *(tmp->DoB) = getDate(tmpStr);
...
bool importStaffs() {
    std::ifstream ifs("CSV/Staff.csv");
    if (ifs.is_open() == false)
        return false;
    std::string str;
    std::getline(ifs, str);
    while (!ifs.eof()) {
        STAFF* tmp = new GROUP1::STAFF;
        std::getline(ifs, str, ','); // get username
        tmp->user->username = marshal_as<System::String^>(str);

        std::getline(ifs, str, ','); // get password
        tmp->user->password = marshal_as<System::String^>(str);

        std::getline(ifs, str, ',');
        tmp->firstname = marshal_as<System::String^>(str);

        std::getline(ifs, str);
        tmp->lastname = marshal_as<System::String^>(str);
...
bool importClasses()
{
    std::ifstream ifs("CSV/Class.csv");
    if (ifs.is_open() == false)
        return false;
    std::string str;
    std::getline(ifs, str);
    while (!ifs.eof())
    {
        ...
        std::getline(ifs, str, ',');
        tmp->program = convertToProgram(marshal_as<System::String^>(str));
...
bool importStudentsInACourse(std::string filename, GROUP1::COURSE* c) {
    ...
    while (!ifs.eof())
    {
        ...
        while (stu != nullptr && marshal_as<System::String^>(str)->Equals(stu->data->studentID) == false )
            stu = stu->next;
...
bool importSchoolYears() {
    ...
    std::ifstream inp("CSV/SchoolYear");
    if (!inp.is_open()) return 0;

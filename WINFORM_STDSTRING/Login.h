...
#pragma endregion
	...
	#pragma region Windows Form Designer generated code
			...
			void InitializeComponent(void)
			{
				...
				this->Name = L"Login";
				this->Enter += gcnew System::EventHandler(this, &Login::btn_signin_Click);
				this->ResumeLayout(false);
				this->PerformLayout();
	...
	#pragma endregion
		...
		private: System::Void Login_SizeChanged(System::Object^ sender, System::EventArgs e) {
			int deltaX = this->Width - this->MinimumSize.Width;
			int deltaY = this->Height - this->MinimumSize.Height;
			int controlX = 1; // X value of the control from step 3
			int controlY = 2; // Y value of the control from step 3
			CMS::Login->Controls["btn_sign_in"] = new Point(controlX + deltaX, controlY + deltaY);
		}
			...
			while (cur) {
				if (input.username == cur->data->user.username) {// cur->data->user.username
					if (input.password == cur->data->user.password) {//cur->data->user.password
						...
						//this->Close();
						// call another form
						//CMS::My22^ Task22 = gcnew CMS::My22();
						//Task22->Show();
						exportSchoolYears();
						exportStudents();
						exportClasses();
